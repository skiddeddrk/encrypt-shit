getgenv().ServerHopTime = 60
getgenv().GodBlock = true
local success, err = pcall(function()
    ...
    if GodBlock then
        for i,v in pairs(game.Workspace.Players[game.Players.LocalPlayer.Name]:GetChildren()) do
        ...
        sfolder.Name = "FULLY_LOADED_CHAR"
        sfolder.Parent = newchar
        newchar:WaitForChild('RagdollConstraints'):Destroy()
        local nval = Instance.new('BoolValue', newchar)
        nval.Name = 'RagdollConstraints'
        wait(2)
        player.Character.BodyEffects.Defense:Destroy()
        local Defense = Instance.new("IntValue", player.Character.BodyEffects)
